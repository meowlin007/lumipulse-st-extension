...
// ── Color palettes for both modes ──────────────────────────
function getThemeVars(colorMode) {
    if (colorMode === 'light') {
        return {
            bg: '#FFF5F8', card: '#FFFFFF', border: 'rgba(255,107,157,0.18)',
            text: '#2D1B2E', sub: '#8B7080', inputBg: '#FFF0F4',
            navBg: '#FFFFFF', heroBg: 'linear-gradient(135deg,#FF6B9D,#FF3D7F)',
            shadow: '0 2px 16px rgba(255,107,157,0.12)',
            postBg: '#FFFFFF', replyBg: '#FFF8FA',
            composeBg: '#FFF5F8'
        };
    }
    return {
        ...
        navBg: '#16101F', heroBg: 'linear-gradient(135deg,#FF6B9D,#FF3D7F)',
        shadow: '0 8px 32px rgba(0,0,0,0.5)',
        postBg: '#1A1128', replyBg: '#130D1C',
        composeBg: '#130D1C'
    };
}

function applyColorMode(colorMode) {
    const v = getThemeVars(colorMode);
    ...
    // primary pink always
    r.style.setProperty('--lp',    '#FF6B9D');
    r.style.setProperty('--ls',    '#FF3D7F');
    r.style.setProperty('--la',    'rgba(255,107,157,0.12)');
    r.style.setProperty('--ldanger','#FF4757');
    r.style.setProperty('--lgrad', 'linear-gradient(135deg,#FF6B9D,#FF3D7F)');
    r.style.setProperty('--ltag',  'rgba(255,107,157,0.15)');
    r.style.setProperty('--ltagtext','#FF9EC4');
    r.style.setProperty('--lglow', '0 0 20px rgba(255,107,157,0.25)');
...
function initLumiPulse() {
    ...
    applyColorMode(EXT._internal.colorMode || 'dark');
    injectStyles();
    createSettingsPanel();
    if (EXT.isEnabled) {
        setTimeout(() => {
...
/* ── FORUM ── */
.lumi-forum-wrap{display:flex;flex-direction:column;height:100%;}
.lumi-forum-feed{padding:10px 12px 14px;overflow-y:auto;flex:1;}
.lumi-post{
...
/* Post special cards */
.lumi-song-card{background:linear-gradient(135deg,rgba(255,107,157,.08),rgba(139,92,246,.08));border:1px solid rgba(255,107,157,.25);border-radius:12px;padding:10px 12px;margin-bottom:8px;display:flex;align-items:center;gap:10px;}
...
/* depth-2 reply */
.lumi-reply-l2{margin-left:10px;padding-left:8px;border-left:1.5px solid rgba(255,107,157,.2);}
...
/* Compose */
.lumi-compose{margin:0 12px 0;background:var(--lcompose);border:1px solid var(--lborder);border-radius:16px;padding:11px;flex-shrink:0;}
...
/* ── Settings ── */
.lumi-theme-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:6px;margin-bottom:4px;}
.lumi-theme-card{border:2px solid var(--lborder);border-radius:10px;padding:8px;cursor:pointer;transition:.18s;display:flex;align-items:center;gap:7px;background:var(--lbg);}
.lumi-theme-card.selected{border-color:var(--lp);background:var(--la);}
.lumi-theme-dot{width:18px;height:18px;border-radius:50%;flex-shrink:0;}
.lumi-theme-label{font-size:10px;font-weight:600;color:var(--lsub);}
...
async function callDiaryAI(mode,count) {
    ...
    // ✅ แก้ prohibited: ใช้ fictional framing ชัดเจน
...
function renderForum() {
    ...
    body.html(`
      ...
      <div class="lumi-compose" style="margin:10px 12px 0;flex-shrink:0;">
      ...
      <div class="lumi-forum-feed" id="lumi-feed" style="padding:10px 12px 14px;overflow-y:auto;"></div>
    `);

    // compose mode toggle
    $('.lumi-mode-btn').on('click',function(){
        $('.lumi-mode-btn').removeClass('active');$(this).addClass('active');
    ...
    $('#btn-forum-clear').on('click',()=>{if(confirm('ลบโพสต์ทั้งหมดของตัวละครนี้?')){savePosts([]);renderFeed();toast('✓ ล้างแล้ว');}});
    ...
    $('#btn-post').on('click',async()=>{
        ...
        if(mode==='image'){
            ...
            const newPost={id:'post_'+Date.now(),type:'thread',posterType:'player',author:ctx.name1||'Player',postStyle:'image',imageData:b64,imageCaption:caption,content:caption||'📷',threadTag:'Photo',timestamp:new Date().toISOString(),parentId:null,likes:0,likedBy:[],replies:[]};
            const arr=botPosts();arr.push(newPost);savePosts(arr);
            $('#compose-img-preview').html(`${I.image} คลิกเพื่อเลือกรูปภาพ`).data('b64','');$('#compose-img-caption').val('');
        } else {
            const txt=$('#compose-txt').val().trim();
            if(!txt){toast('เขียนอะไรก่อนนะคะ','err');return;}
            const newPost={id:'post_'+Date.now(),type:'thread',posterType:'player',author:ctx.name1||'Player',postStyle:'text',content:txt,threadTag:'Discussion',timestamp:new Date().toISOString(),parentId:null,likes:0,likedBy:[],replies:[]};
            const arr=botPosts();arr.push(newPost);savePosts(arr);
            $('#compose-txt').val('');
        }
        renderFeed();
        setTimeout(()=>aiReactToPost(botPosts().slice(-1)[0]?.id),1200);
    });
...
// ── Feed ──────────────────────────────────────────────────
function renderFeed() {
    if(!$('#lumi-feed').length) return; // ✅ guard: ป้องกัน error ตอน DOM ยังไม่พร้อม
    const threads=botPosts().filter(p=>p.type==='thread').sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp));
    if(!threads.length){
        $('#lumi-feed').html(`<div class="lumi-empty"><div class="lumi-empty-icon">💬</div><div class="lumi-empty-text">ยังไม่มีโพสต์<br>กด Refresh Feed เพื่อดูว่ามีอะไรเกิดขึ้นบ้าง!</div></div>`);
        return;
    }
    let html='';
    threads.forEach(th=>{
...
function postCardHTML(thread, allReplies) {
    ...
    const typeClass=thread.posterType==='news'?'post-news':thread.posterType==='citizen'?'post-citizen':'';
    const typeBadge=thread.posterType==='news'?`<span class="lumi-type-badge badge-news">${I.news} ข่าว</span>`:thread.posterType==='npc'?`<span class="lumi-type-badge badge-npc">NPC</span>`:thread.posterType==='citizen'?`<span class="lumi-type-badge badge-citizen">ประชาชน</span>`:`<span class="lumi-type-badge badge-main">หลัก</span>`;
    ...
    // ✅ Threaded replies — รองรับ 3 ชั้น
...
function bindFeedEvents() {
    ...
    $('[data-act="toggle-replies"]').off('click').on('click',function(){
        const id=$(this).data('id');$(`#rep-${id}`).is(':visible')?$(`#rep-${id}`).slideUp(160):$(`#rep-${id}`).slideDown(180);
    });
    $('[data-act="reply-to-reply"]').off('click').on('click',function(){
        const rid=$(this).data('rid');
        $('[class^="lumi-compose-l2"]').not(`#l2-${rid}`).hide();
        $(`#l2-${rid}`).toggle();
    });
    ...
    $('[data-act="del-post"]').off('click').on('click',function(e){
        ...
        savePosts(botPosts().filter(p=>p.id!==id&&p.parentId!==id));renderFeed();
    });
...
- Sharing personal feelings/moods (เศร้า ดีใจ เหนื่อย ตื่นเต้น)
...
- Posting a photo description (use postStyle:"image_desc" — NOT actual image, just describe what they'd post)
- Casual conversations and jokes

Each post can have 2-5 replies from different characters that have natural back-and-forth conversations.
...
[
  {
    ...
    try {
        ...
        data.filter(p=>p&&(p.content||p.songTitle||p.imageDesc)).forEach((p,i)=>{
            ...
            if(postStyle==='image_desc'){
                postStyle='image_desc';
                extraFields={imageDesc:p.imageDesc||''};
            }
            toAdd.push({
        ...
        renderFeed(); // ✅ เรียกหลัง savePosts เสมอ
        toast(`✓ เพิ่ม ${toAdd.filter(p=>p.type==='thread').length} โพสต์`);

        // Mark unread if forum not open
...
async function aiReactToPost(threadId) {
    ...
    const numRep=Math.min(3,1+Math.floor(Math.random()*3)); // 1-3 replies
...
function setupForumAutoTrigger() {
    $(document).off('messageReceived.lumi-forum').on('messageReceived.lumi-forum', async()=>{
        ...
        // ✅ ถ้ามีโพสต์ที่โพสต์ไว้แล้ว ให้ AI มา react ด้วย (ประหยัด call)
...
function renderSettings() {
    ...
    <div class="lumi-form-card">
      ...
      <div class="lumi-set-row">
        ...
        <div class="lumi-cm-toggle">
          ...
          <div class="lumi-cm-track${cm==='dark'?' dark-mode':''}" id="cm-track">
    ...
    $('#cm-track').on('click',function(){
        ...
        EXT._internal.colorMode=newMode;save();
    ...
    $('#api-provider').on('change',function(){
        EXT.api.provider=$(this).val();save();
        $('#url-row').toggle(EXT.api.provider!=='google'&&EXT.api.provider!=='anthropic');
        $('#model-dropdown-wrap').hide();
    });
