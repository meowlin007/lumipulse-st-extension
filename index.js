...
function initLumiPulse() {
    ...
    if (extension_settings[extensionName].isEnabled) {
        setTimeout(() => {
            spawnLumiButton();
            createModal();
            setupAutoTriggerListener(); // ✅ ฟังก์ชันเดียวจัดการทั้ง diary + forum
        }, 500);
    }
}
...
.lumi-card { background:var(--lumi-card); border:1px solid var(--lumi-border); border-radius:16px; padding:14px; margin:0 0 10px 38px; position:relative; transition:transform 0.2s,box-shadow 0.2s; color:var(--lumi-text); }
.lumi-card:hover { box-shadow:0 8px 20px rgba(255,105,180,0.15); transform:translateY(-3px); }
...
function spawnLumiButton() {
    ...
    $(document).off('click','#lumi-open').on('click','#lumi-open',     () => { $(menu).fadeOut(); openModal('diary'); });
    $(document).off('click','#lumi-forum-btn').on('click','#lumi-forum-btn', () => { $(menu).fadeOut(); openModal('forum'); });
...
function renderDiaryTab() {
    ...
    if (filterLoc)  filteredMems = filteredMems.filter(m => m.content.rp_location === filterLoc);
    const byDate = {};
    filteredMems.forEach(m => { const date = m.content.rp_date || 'Unknown Date'; if(!byDate[date]) byDate[date]=[]; byDate[date].push(m); });
    const sortedDates = Object.keys(byDate).sort();
    let html = sortedDates.length === 0 ? `<div style="text-align:center;padding:60px 20px;animation:fadeIn 0.5s ease;"><div style="font-size:64px;margin-bottom:16px;opacity:0.3;color:var(--lumi-primary)">${svgCalendar}</div><div style="font-size:16px;color:#999;">No memories yet</div></div>` : '';
    sortedDates.forEach(date => { const entries = byDate[date]; if(!entries.length) return; html += `<div class="lumi-timeline-date"><div style="font-size:13px;color:var(--lumi-secondary);font-weight:500;display:flex;align-items:center;gap:6px">${svgCalendar} ${date}</div></div>`; entries.forEach((m,idx)=>{ html += renderCard(m,idx); }); });
...
function renderCard(m, index) {
    ...
    const locHtml  = m.content.rp_location ? `<span class="lumi-badge" style="cursor:pointer">${svgMapPin} ${escapeHtml(m.content.rp_location)}</span>` : '';
    ...
    const tagsHtml = (m.content.rp_tags && m.content.rp_tags.length) ? `<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px;">${m.content.rp_tags.map(t=>`<span class="lumi-badge" style="font-size:10px;">${svgTag} ${t}</span>`).join('')}</div>` : '';
...
async function weaveStory(mems) {
    ...
    const prompt = `[System: Weave these diary entries into a cohesive narrative story in Markdown.]\n${diaryText}`;
    try {
        let res;
...
function renderLoreExtractor() {
    ...
    $('#btn-extract-lore').on('click', async function() {
        ...
        } else { $('#lore-output').html('<div style="text-align:center;padding:20px;color:#999;">No extractable lore found.</div>').show(); }
    });
}
...
async function extractLore(mems) {
    ...
    const prompt = `[System: Extract World Info for SillyTavern Lorebook.]\nText:\n${text}\nReturn ONLY JSON array:\n[{"keyword":"Name/Place/Item","type":"character|location|item|event|rule","content":"Brief description"}]`;
    try {
        let res;
        if(typeof ctx.generateQuietPrompt==='function') res = await ctx.generateQuietPrompt(prompt,false,false);
        else if(typeof ctx.generateRaw==='function') res = await ctx.generateRaw(prompt,true);
        const match = res?.match(/\[[\s\S]*\]/); if(!match) return {entries:{}};
        const aiData = JSON.parse(match[0]); const entries = {};
        aiData.forEach((item,index) => { entries[index] = { uid:index, key:[item.keyword], keysecondary:[], comment:item.type, content:item.content, constant:false, selective:true, selectiveLogic:0, order:10, position:0, disable:false, probability:100, useProbability:true, depth:4, group:"LumiPulse Extracted" }; });
        return {entries};
...
function renderMemoryLinks() {
    ...
    let html = linkedMems.length === 0 ? `<div style="text-align:center;padding:40px;color:#999;">No linked memories yet.</div>` : '';
    linkedMems.forEach(m => {
        const links = m.meta.linkedIds.map(id => { const linked = mems.find(x=>x.id===id); return linked ? `<div class="lumi-badge" style="margin:4px 0;cursor:pointer" data-id="${linked.id}">${svgLink} ${linked.character} | ${linked.content.rp_date}</div>` : ''; }).join('');
    ...
    $('.lumi-badge[data-id]').off('click').on('click', function() {
        const id = $(this).data('id'); const mem = mems.find(m=>m.id===id);
        if(mem) { $('#lumi-content').html(renderCard(mem,0)+`<div style="text-align:center;margin-top:15px;"><button id="back-links" class="lumi-gen-btn">${svgBack} Back to Links</button></div>`); $('#back-links').off('click').on('click', ()=>renderMemoryLinks()); }
    });
}
...
function renderForum() {
    ...
    $('#lumi-body').html(`
        ...
        <div class="lumi-nav">
            ...
            <div class="lumi-nav-tab" data-forum-tab="analytics">${svgMood} Analytics</div>
        </div>
        <div class="lumi-action-row" style="margin-bottom:15px;">
            <div class="lumi-filters">
                <select id="forum-filter-char" class="lumi-filter-select">
                    <option value="">All Characters</option>
                    ${chars.map(c=>`<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join('')}
                </select>
                <select id="forum-filter-type" class="lumi-filter-select">
    ...
    // ✅ แก้บัก 2: ใช้ filter ที่อยู่ใน DOM จริง ไม่ error เมื่อเรียกจากนอก modal
    $('#forum-filter-char, #forum-filter-type').on('change', () => renderForumThreads());

    // ✅ แก้บัก 1: caller จัดการ render — generate/submit ไม่เรียก renderForum()
    ...
    $('.lumi-nav-tab[data-forum-tab]').on('click', function() {
        $('.lumi-nav-tab[data-forum-tab]').removeClass('active'); $(this).addClass('active');
...
function renderForumThreads() {
    // ✅ แก้บัก 2: guard — ถ้า forum ไม่ได้เปิดอยู่ ไม่ต้อง render
    if (!$('#forum-main-content').length) return;

    const posts = extension_settings[extensionName].forumPosts || [];
    const filterChar = $('#forum-filter-char').val() || '';
    const filterType = $('#forum-filter-type').val() || 'all';
...
function renderForumPost(post) {
    ...
    return `<div class="lumi-forum-post" style="${isPlayer?'border-left:3px solid var(--lumi-primary);':''}">
...
function renderForumNetwork() {
    ...
    chars.forEach(char => {
        ...
        html += `<div class="lumi-network-node" data-char="${escapeHtml(char)}"><div style="font-size:11px;font-weight:500;">${escapeHtml(char)}</div><div style="font-size:10px;color:#888;">${postCount} posts · Affinity: ${affinity}</div></div>`;
    });
    $('#network-nodes').html(html);
    $('.lumi-network-node').off('click').on('click', function() { showCharacterDetails($(this).data('char')); });
}
...
function renderForumAnalytics() {
    ...
    let html = `<div style="padding:15px;">
        ...
        <div class="lumi-stats-bar"><div class="lumi-stat"><b>${posts.length}</b><span>Total Posts</span></div><div class="lumi-stat"><b>${Object.keys(moodData).length}</b><span>Moods</span></div></div>
        <div style="margin-top:15px;">
            ${Object.entries(moodData).map(([mood,count]) => `
                <div style="margin:8px 0;padding:10px;background:var(--lumi-bg);border-radius:8px;">
                    <div style="display:flex;justify-content:space-between;font-size:12px;"><span>${mood}</span><span style="color:var(--lumi-primary);font-weight:500;">${count} posts</span></div>
                    <div style="height:6px;background:var(--lumi-border);border-radius:3px;margin-top:5px;overflow:hidden;"><div style="height:100%;background:var(--lumi-primary);width:${(count/posts.length)*100}%;transition:width 0.3s;"></div></div>
                </div>`).join('')}
        </div>
    </div>`;
...
function groupPostsByThread(posts) {
    const threads = {};
    posts.forEach(post => {
        if(!threads[post.threadId]) threads[post.threadId] = { title: post.threadTitle || 'Discussion', posts: [] };
...
function analyzeCharacterConnections(posts) {
    const connections = {};
    posts.forEach(post => { if(post.author && post.author!=='Player') connections[post.author] = (connections[post.author]||0)+1; });
    return connections;
}

function analyzeForumMoods(posts) {
    const moods = {};
    posts.forEach(post => { const mood = detectMood(post.content); moods[mood] = (moods[mood]||0)+1; });
    return moods;
}
...
function showCharacterDetails(char) {
    ...
    const affinity = Math.min(100, charPosts.length * 5);
    ...
    `);
    $('#back-to-network').on('click', () => { renderForumThreads(); renderForumNetwork(); });
}

// ✅ แก้บัก 1: generate/submit คืนค่า true/false เท่านั้น ไม่เรียก renderForum()
async function generateForumPosts() {
    ...
    if(characters.length === 0) { showToast('No characters in recent chat'); return false; }
...
Generate 2-3 forum posts from different characters discussing recent events.
Return JSON array:
[{"author":"Character Name","threadId":"1","threadTitle":"Topic Title","content":"Post content","type":"public","mood":"Happy|Angry|Sad|Neutral","timestamp":"${new Date().toISOString()}"}]`;
    ...
    try {
        ...
        return true;
    } catch(e) { console.error(e); showToast('Error generating forum posts'); return false; }
}
...
async function submitForumPost() {
    ...
    const s = extension_settings[extensionName];
    const newPost = { id:'post_'+Date.now(), author:'Player', threadId:'player_'+Date.now(), threadTitle:'Player Discussion', content, type, mood:'Neutral', timestamp:new Date().toISOString(), replies:[] };
...
// ═══════════════════════════════════════════════
// 11. AUTO-TRIGGER — ✅ แก้บัก 1: รวม diary + forum ไว้ที่เดียว
// ═══════════════════════════════════════════════
    ...
    if(cfg.enabled) {
        ...
        if(gen) {
            ...
            if(results && results.length > 0) {
                ...
                results.forEach(res => saveMemory({ id:'mem_auto_'+Date.now()+'_'+Math.random().toString(36).substr(2,5), timestamp:new Date().toISOString(), character:res.character||ctx.name2||"Character", botId:ctx.characterId, worldMode:wm, content:{...res}, meta:{isPinned:false,isFavorite:false,isSecret:res.isSecret,refRange:res.refRange,linkedIds:res.linkedIds||[],tags:extractTags(res.diary)} }));
            }
        }
    }

    // 💬 Forum Auto-Gen — ✅ แก้บัก 2: renderForumThreads() มี guard ป้องกัน error
    const fcfg = s.forum.autoGen;
    if(s.forum.enabled && fcfg.enabled) {
        s._internal.forumMessageCounter = (s._internal.forumMessageCounter || 0) + 1;
        const now = Date.now();
        let shouldGenerate = false;
        if(s._internal.forumMessageCounter >= fcfg.messageInterval) { shouldGenerate=true; s._internal.forumMessageCounter=0; }
        if(now - (s._internal.lastForumGenTime||0) >= fcfg.timeInterval * 60 * 1000) { shouldGenerate=true; }
        if(fcfg.onImportantEvent) {
            const lastMsg = (SillyTavern.getContext().chat?.slice(-1)[0]?.mes||'').toLowerCase();
            if(['รัก','แต่งงาน','ตาย','เจ็บ','สำคัญ','ความลับ','งานเลี้ยง'].some(k=>lastMsg.includes(k))) shouldGenerate=true;
        }
        if(shouldGenerate) {
            s._internal.lastForumGenTime = now;
            const success = await generateForumPosts();
            // ✅ renderForumThreads() มี guard — เรียกได้เลยแม้ modal ปิดอยู่
            if(success) renderForumThreads();
        }
    }
}
...
${chatLog}
Rules:
Focus ONLY on events within #${startIndex+1}-#${endIndex}.
Return refRange: {start:${startIndex+1},end:${endIndex}}.
Link to related existing memory IDs if applicable.
Date MUST be numeric Thai format.
    ...
    $('#lumi-body').html(`
        <div style="padding:10px;">
            ...
            <div style="margin-top:15px;display:flex;gap:10px;">
                ...
                <button id="btn-clr" class="lumi-input" style="background:var(--lumi-danger);color:white;text-align:center;cursor:pointer;border:none;">${svgClose} Clear Data</button>
            </div>
        </div>
    `);
    // ✅ .data() หลัง .html()
...
function levenshteinDistance(str1,str2) { const m=str1.length,n=str2.length; const dp=Array(m+1).fill(null).map(()=>Array(n+1).fill(0)); for(let i=0;i<=m;i++) dp[i][0]=i; for(let j=0;j<=n;j++) dp[0][j]=j; for(let i=1;i<=m;i++) for(let j=1;j<=n;j++) dp[i][j]=str1[i-1]===str2[j-1]?dp[i-1][j-1]:1+Math.min(dp[i-1][j],dp[i][j-1],dp[i-1][j-1]); return dp[m][n]; }
function similarityScore(str1,str2) { const s1=str1.toLowerCase().trim(),s2=str2.toLowerCase().trim(); const distance=levenshteinDistance(s1,s2); const maxLen=Math.max(s1.length,s2.length); return maxLen===0?100:((maxLen-distance)/maxLen)*100; }

function editMemoryInline(id) {
    const mem=extension_settings[extensionName].memories.find(m=>m.id===id); if(!mem) return;
    const card=$(`.lumi-card[data-id="${id}"]`);
    card.find('.lumi-text').html(`<textarea class="lumi-edit-textarea" style="width:100%;min-height:80px;padding:10px;border:1px solid var(--lumi-border);border-radius:10px;font-family:'Mitr';font-size:13px;resize:vertical;color:var(--lumi-text);background:var(--lumi-card)">${mem.content.diary}</textarea><div style="margin-top:8px;display:flex;gap:8px"><button class="lumi-btn-save" style="flex:1;background:var(--lumi-primary);color:white;border:none;padding:8px;border-radius:8px;cursor:pointer">Save</button><button class="lumi-btn-cancel" style="flex:1;background:#FFE0E0;color:var(--lumi-danger);border:none;padding:8px;border-radius:8px;cursor:pointer">Cancel</button></div>`);
    card.find('.lumi-btn-save').on('click', function() { mem.content.diary=card.find('.lumi-edit-textarea').val(); SillyTavern.getContext().saveSettingsDebounced(); renderDiaryTab(); showToast('Updated!'); });
    card.find('.lumi-btn-cancel').on('click', function() { renderDiaryTab(); });
}

function editMemoryModal(id) {
    const mem=extension_settings[extensionName].memories.find(m=>m.id===id); if(!mem) return;
    $('#lumi-title').text('Edit Memory');
    $('#lumi-body').html(`<div style="padding:15px;"><div class="lumi-form"><label class="lumi-label">Character</label><input type="text" id="edit-char" value="${mem.character}" class="lumi-input"></div><div class="lumi-form"><label class="lumi-label">Date (RP)</label><input type="text" id="edit-date" value="${mem.content.rp_date||''}" class="lumi-input"></div><div class="lumi-form"><label class="lumi-label">Location</label><input type="text" id="edit-loc" value="${mem.content.rp_location||''}" class="lumi-input"></div><div class="lumi-form"><label class="lumi-label">Tags</label><input type="text" id="edit-tags" value="${(mem.content.rp_tags||[]).join(', ')}" class="lumi-input"></div><div class="lumi-form"><label class="lumi-label">Diary</label><textarea id="edit-diary" class="lumi-input" style="min-height:150px;resize:vertical">${mem.content.diary}</textarea></div><div style="display:flex;gap:10px"><button id="btn-save-edit" class="lumi-gen-btn" style="flex:2">Save</button><button id="btn-cancel-edit" class="lumi-input" style="flex:1;background:#FFE0E0;color:var(--lumi-danger);text-align:center;cursor:pointer">Cancel</button></div></div>`);
    $('#btn-save-edit').on('click', function() { mem.character=$('#edit-char').val(); mem.content.rp_date=$('#edit-date').val(); mem.content.rp_location=$('#edit-loc').val(); mem.content.rp_tags=$('#edit-tags').val().split(',').map(t=>t.trim()).filter(t=>t); mem.content.diary=$('#edit-diary').val(); SillyTavern.getContext().saveSettingsDebounced(); renderDashboard(); showToast('Updated!'); });
    $('#btn-cancel-edit').on('click', function() { renderDashboard(); });
}
