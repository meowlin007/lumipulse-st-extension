...
function initLumiPulse() {
    ...
    // ❌ ลบ global click listener ที่อาจซ้อนกับ heart effect เดิม
    // ✅ ใช้ระบบเดิมของคุณที่ผูกกับปุ่มเท่านั้น
}
// ═══════════════════════════════════════════════
// HELPERS
// ═══════════════════════════════════════════════
...
async function requestAIGeneration(topic, npcs, includeRandom) {
    ...
    const randomNote = includeRandom        ? 'คุณสามารถสร้างชื่อ NPC เสริมที่ไม่มีในรายชื่อได้'
...
// 🎭 แก้ไข: requestDiaryGeneration - เพิ่ม Personality Injection
async function requestDiaryGeneration() {
    const charName = getCharacterName();
    const userName = getUserName();
    const chatLog  = getRecentChatSummary();
    
    // 🔍 ดึงข้อมูลตัวละครจาก SillyTavern Context
    const ctx = SillyTavern.getContext();
...
=== INSTRUCTIONS ===
You are ${charName}. Based on the conversation log above, write your PRIVATE diary entry about your feelings toward ${userName}.

Rules:1. Write in Thai language, casual diary style, emotional and intimate
...
4. Keep it short (3-5 sentences), like a real personal diary
5. Add 1-2 emojis naturally ONLY if it fits ${charName}'s personality
...
function injectStyles() {
    ...
    style.innerHTML = `
        ...
        #lumi-main-fab {
            ...
            /* 🚀 ปรับปรุงประสิทธิภาพการลากบนมือถือ */
        ...
        .lumi-modal-close {            position: absolute; right: 20px; top: 25px;
...
// FORUM UI
// ═══════════════════════════════════════════════
function renderForumUI() {    const s    = extension_settings[extensionName];
...
const AFFECTION_LEVELS = [
    { min: 0,  max: 20,  label: "แปลกหน้า",   color: "#CCC",    emoji: "😶" },    { min: 20, max: 40,  label: "รู้จักกัน",    color: "#FFD1DC", emoji: "🙂" },
...
// ═══════════════════════════════════════════════
// FAB BUTTON - 🐛 แก้ไข: ลากลื่น + แตะง่ายบนมือถือ
// ═══════════════════════════════════════════════
function spawnLumiButton() {
    ...
    // 🎯 เพิ่ม attribute สำหรับ accessibility
    ...
    fab.addEventListener('mousedown', (e) => {
        ...
        function onMouseMove(ev) {
            ...
            fab.style.left  = x + 'px';
            fab.style.top   = y + 'px';
            fab.style.right = 'auto';            updateMenuPos();
        }
        ...
        function onMouseUp(ev) {
            ...
            // ถ้าไม่ได้ลาก = แตะปกติ (เปิดเมนู)
            if (!isDragging) {
                clearTimeout(tapTimer);
                tapTimer = setTimeout(() => {
                    updateMenuPos();
                    $(menu).fadeToggle(300);
                    // ✅ ใช้ _fromDrag เพื่อไม่ให้หัวใจเด้งซ้ำ
                    const fakeEvent = { ...ev, _fromDrag: false, clientX: ev.clientX, clientY: ev.clientY };
                    spawnHeartEffect(fakeEvent);
                }, 50);
            } else {
                // จบการลาก = บันทึกตำแหน่ง (optional)
                updateMenuPos();
            }
            isDragging = false;
        }
    ...
    // 📱 Touch events (Mobile) - 🚀 ปรับปรุงประสิทธิภาพ
    let touchOffset = { x: 0, y: 0 };
    let isTouchDrag = false;

    fab.addEventListener('touchstart', (e) => {
        e.preventDefault(); // ป้องกัน scroll ขณะแตะ
        isTouchDrag = false;
        fab.classList.remove('lumi-floating');
    ...
    fab.addEventListener('touchend', (e) => {
        ...
        if (!isTouchDrag) {
            ...
            tapTimer = setTimeout(() => {
                ...
                if (t) {
                    const fakeEvent = { _fromDrag: false, clientX: t.clientX, clientY: t.clientY };
                    spawnHeartEffect(fakeEvent);
                }
            }, 50);
        }
    ...
    // ⌨️ Keyboard support (Accessibility)
    ...
    // 🔄 Long press = reset position (optional)
    ...
    fab.addEventListener('touchstart', () => {
        longPressTimer = setTimeout(() => {
            if (!isTouchDrag) {
                ...
                fab.style.right = '20px';
                fab.style.left = 'auto';
                toastr.info('📍 รีเซ็ตตำแหน่งปุ่มแล้ว', 'LumiPulse');
            }
        }, 800);
    }, { passive: true });
