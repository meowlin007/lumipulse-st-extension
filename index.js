...
// ═══════════════════════════════════════════════
// BOOT — แยกออกจาก comment ให้ชัดเจน
// ═══════════════════════════════════════════════
jQuery(async () => {
...
async function callST(p) {
    ...
    } catch (e) { console.error('[LumiPulse]', e); toastr.error('AI Error: ' + e.message); return null; }
}

async function requestDiaryGen(opt = {}) {
    const { trigger = 'manual', messageRange = 30, useAllMessages = false, startFromIndex = 0 } = opt;
    const ctx = SillyTavern.getContext();
    ...
    const chatLog = chatSlice.map((m, i) => `[#${startIndex + i + 1}] ${m.is_user ? userName : (m.name || 'NPC')}: ${(m.mes || '').slice(0, 150)}`).join('\n');
...
async function onNewChat() {
    ...
    else if (lastMsg.includes('#จำ') || lastMsg.includes('#diary')) { gen = true; type = 'user_tag'; }
    if (!gen) return;
    ...
    if (res) {
        ...
        createMemoryEntry(res, type, ctx, wm, ctx.chat?.slice(-1)[0]?.mes || '', ctx.chat?.length || 0);
    }
}
...
async function manualGenerate() {
    ...
    $('#gen-confirm').on('click', async () => {
        ...
        const res = await requestDiaryGen({ trigger: 'manual', messageRange, useAllMessages, startFromIndex });
        if (res) {
            const ctx2 = SillyTavern.getContext();
            createMemoryEntry(res, 'manual', ctx2, wm, ctx2.chat?.slice(-1)[0]?.mes || '', ctx2.chat?.length || 0);
            showToast('บันทึกความทรงจำเสร็จแล้ว', 'success');
            setTimeout(() => loadAndRenderTimeline(), 1000);
        } else {
...
function injectStyles() {
    ...
    s.innerHTML = `
        ...
        #lumi-main-fab {
            ...
            background:#FFF0F5 url('${btnUrl}') no-repeat center center!important;
        ...
        #lumi-main-fab:active { cursor:grabbing!important; }
...
function createContentModal() {
    ...
    `);
    $('#lumi-modal-back').on('click', () => loadAndRenderTimeline());
...
function renderDashboard() {
    ...
    const chars = [...new Set(mem.map(m => m.character))];
    $('#lumi-modal-body').html(`
        <div style="padding:16px;">
            <div class="lumi-stats-grid">
                ...
                <div class="lumi-stat-card"><div class="lumi-stat-value">${mem.filter(m=>m.meta.isPinned).length}</div><div class="lumi-stat-label">ปักหมุด</div></div>
                <div class="lumi-stat-card"><div class="lumi-stat-value">${mem.filter(m=>m.meta.isFavorite).length}</div><div class="lumi-stat-label">ล้ำค่า ⭐</div></div>
                <div class="lumi-stat-card"><div class="lumi-stat-value">${mem.filter(m=>m.meta.isSecret).length}</div><div class="lumi-stat-label">ความลับ</div></div>
            </div>
            <div class="lumi-settings-card">
                ...
                ${chars.map(char => {
                    ...
                    return `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px;font-size:11px;color:#666;">
                        <span style="display:flex;align-items:center;gap:5px;"><span class="lumi-char-dot" style="background:${color}"></span>${escapeHtml(char)}</span>
                        <span style="color:#ff85a2;">${count}</span>
                    </div>`;
                }).join('')}
                ${chars.length === 0 ? '<div style="font-size:11px;color:#ccc;text-align:center;">ยังไม่มีความทรงจำ</div>' : ''}
            </div>
    ...
    $('#lumi-modal-body').html(`
        ...
        <div class="lumi-filter-bar">
            <select id="lumi-f-char" class="lumi-filter-select">
                ...
                ${chars.map(c => `<option value="${escapeHtml(c.name)}">${escapeHtml(c.name)}</option>`).join('')}
            </select>
...
function loadAndRenderTimeline() {
    ...
    const grouped = {};
    mem.forEach(m => { const dk = m.content.rp_date || 'ไม่ระบุวันที่'; if (!grouped[dk]) grouped[dk] = []; grouped[dk].push(m); });
    let h = '';
...
function renderMemoryCard(entry, compact = false) {
    ...
    const acts = `
        <button class="lumi-btn-icon ${entry.meta.isPinned ? 'active' : ''}" data-act="pin">${svgPin}</button>
    ...
    `;
    if (isLocked) return `<div class="lumi-memory-card secret-locked" data-id="${entry.id}">
...
function exportMemories() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `lumipulse-memories-${new Date().toISOString().split('T')[0]}.json`; a.click(); URL.revokeObjectURL(url);
...
function createSettingsUI() {
    ...
    $('#lumi_reset_fab').on('click', () => { saveFabPosition(null); $('#lumi-main-fab, .lumi-menu-container').remove(); spawnLumiButton(); showToast('รีเซ็ตปุ่มแล้ว'); });
    $('#lumi_export').on('click', () => exportMemories());
    $('#lumi_clear_memories').on('click', () => { if (confirm('ล้างความทรงจำทั้งหมด?')) { extension_settings[extensionName].memories = []; SillyTavern.getContext().saveSettingsDebounced(); showToast('ล้างแล้ว'); } });
}
...
function spawnLumiButton() {
    ...
    fab.addEventListener('mousedown', e => {
        ...
        const onUp = ev => {
            ...
            else { saveFabPosition({ top: currentPos.y + 'px', left: currentPos.x + 'px', right: 'auto', bottom: 'auto', transform: 'none' }); }
            isDragging = false; hasMoved = false;
        };
    ...
    // Touch
    let isTouchDrag = false, hasTouchMoved = false;
    fab.addEventListener('touchstart', e => {
        e.preventDefault();
        isTouchDrag = false; hasTouchMoved = false;
        fab.classList.add('dragging');
        const touch = e.touches[0], rect = fab.getBoundingClientRect();
    ...
    fab.addEventListener('touchmove', e => {
        ...
        if (dist > TH && !hasTouchMoved) {
            hasTouchMoved = true; isTouchDrag = true;
            $(menu).fadeOut(50);
            fab.style.transform = 'none'; fab.style.right = 'auto'; fab.style.bottom = 'auto';
        }
    ...
    fab.addEventListener('touchend', e => {
        e.preventDefault();
        fab.classList.remove('dragging');
        if (!hasTouchMoved) { updateMenuPos(); $(menu).fadeToggle(150); }
        else { saveFabPosition({ top: currentPos.y + 'px', left: currentPos.x + 'px', right: 'auto', bottom: 'auto', transform: 'none' }); }
        isTouchDrag = false; hasTouchMoved = false;
    }, { passive: false });
