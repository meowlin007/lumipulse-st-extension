...
// ✅ FIXED: SVGs corrected (removed broken template string spacing)
...
function injectStyles() {
    ...
    s.id = 'lumi-styles';
    // ✅ FIXED: CSS corrected (removed spaces in properties like background-size, justify-content)
    s.innerHTML = `
...
function spawnLumiButton() {
    ...
    // ✅ FIXED: Removed extra spaces in class names and attributes
    menu.innerHTML = `
        <div class="lumi-menu-grid">
            <div class="lumi-menu-item" id="lumi-open">
                <img src="${iconDiary}">
                <span>Diary</span>
            </div>
            <div class="lumi-menu-item" id="lumi-forum-btn">
                <img src="${iconForum}">
                <span>Forum</span>
            </div>
            <div class="lumi-menu-item" id="lumi-set">
...
// Diary Dashboard - ✅ ย้าย .data() มาหลัง .html()
...
function renderCard(m, index) {
    ...
    if(isLocked) lockOverlay = `
        <div style="position:absolute;inset:0;background:rgba(255,255,255,0.9);display:flex;flex-direction:column;align-items:center;justify-content:center;border-radius:16px;z-index:1;backdrop-filter:blur(5px);">
            ${svgLock}
            <div style="font-size:11px;color:var(--lumi-secondary);margin-top:5px">Locked</div>
        </div>`;
    ...
    $('#gen-form-container').html(`
        <div class="lumi-form">
            ...
            <div class="lumi-radio-group">
                <label class="lumi-radio-label">
                    <input type="radio" name="gen-mode" value="latest" checked> Latest X
                </label>
                <label class="lumi-radio-label">
                    <input type="radio" name="gen-mode" value="first"> First X
                </label>
                <label class="lumi-radio-label">
                    <input type="radio" name="gen-mode" value="all"> All History
                </label>
            </div>
    ...
    $('#lumi-content').html(`
        <div class="lumi-form">
            ...
            <div class="lumi-set-row">
                <span>Include All Characters</span>
                <input type="checkbox" id="sw-all-chars" checked style="width:20px;height:20px;accent-color:var(--lumi-primary)">
            </div>
            <div class="lumi-set-row">
                <span>Chapter Length</span>
                <select id="sw-chapters" class="lumi-input" style="width:100px">
...
Output as Markdown.
Keep original character voices but enhance flow.`;
    try {
        let res;
...
function renderMemoryLinks() {
    ...
    linkedMems.forEach(m => {
        ...
        html += `<div class="lumi-card" style="margin-bottom:15px;">
            <div class="lumi-meta">
                <span class="lumi-badge lumi-char-badge">${m.character}</span>
                <span class="lumi-badge">${m.content.rp_date}</span>
            </div>
            <div style="font-size:12px;color:#666;margin-bottom:8px;">Linked Memories:</div>
            ${links}
        </div>`;
    });
...
// ═══════════════════════════════════════════════
// FORUM MODE - ✅ แก้ไขแล้ว: ลบ renderForum() ออกจาก generate/submit
// ═══════════════════════════════════════════════
function renderForum() {
    ...
    $('#lumi-body').html(`
        ...
        <div class="lumi-form" style="margin-bottom:15px;">
            ...
            <div style="display:flex;gap:8px;">
                <select id="forum-post-type" class="lumi-filter-select" style="flex:1">
                    <option value="public">Public</option>
                    <option value="private">Private</option>
                </select>
    ...
    // ✅ Bind Events - แก้ไข: เรียก renderForumThreads() แทน renderForum()
    ...
    $('#btn-generate-forum').on('click', async function() {
        ...
        if (success) renderForumThreads(); // ✅ แก้ไข: เรียกแค่ renderForumThreads()
...
function setupForumAutoTrigger() {
    $(document).on('messageReceived', async function() {
        ...
        if(shouldGenerate && s.forum.mode === 'auto') {
            ...
            renderForumThreads(); // ✅ เรียกแค่ renderForumThreads()
        }
    });
}
...
async function onNewChat() {
    ...
    if (gen) {
        ...
        if(results && results.length > 0) {
            ...
            results.forEach(res => saveMemory({ 
                id: 'mem_auto'+Date.now()+'_'+Math.random().toString(36).substr(2,5), 
                timestamp: new Date().toISOString(), 
                character: res.character || ctx.name2 || "Character", 
                botId: botId, 
                worldMode: wm, 
...
// ═══════════════════════════════════════════════
// SETTINGS - ✅ ย้าย .data() มาหลัง .html()
// ═══════════════════════════════════════════════
        ...
        <div style="padding:10px;">
            ...
            <div class="lumi-form">
                ...
                <div class="lumi-set-row">
                    ...
                    <input type="checkbox" id="set-en" ${s.isEnabled?'checked':''} style="width:20px;height:20px;accent-color:var(--lumi-primary)">
                </div>
                <div class="lumi-set-row">
