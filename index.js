...
function getRPGCharacters(limit = 10) {
    ...
    if (currentChar) {
        characters.push({ name: currentChar.name || getCharacterName(), color: generateColorFromString(currentChar.name || 'default') });
    }
    if (ctx.characters) {
        Object.values(ctx.characters).forEach(char => {
            if (char.name && char.name !== currentChar?.name && !characters.find(c => c.name === char.name)) {
                characters.push({ name: char.name, color: generateColorFromString(char.name) });
            }
        });
    }
...
function saveFabPosition(pos) {
    ...
    extension_settings[extensionName]._internal.fabPosition = {
        left: pos.left || 'auto', top: pos.top || 'auto', 
...
function loadFabPosition() {
    const saved = extension_settings[extensionName]._internal.fabPosition;
    if (saved && (saved.left || saved.top)) return saved;
    return { top: '50%', right: '0px', transform: 'translateY(-50%)' };
}

function scrollToMessage(messageIndex) {
    const ctx = SillyTavern.getContext();
    if (!ctx.chat || !ctx.chat[messageIndex]) { showToast('ไม่พบข้อความ'); return; }
...
Profile: ${cc.data?.personality||'Normal'} | World: ${wm==='rpg'?'Group':'Solo'} | Others: ${charsInContext.join(',')||'None'}
Chat Log:\n${chatLog}

Generate a diary entry based on the RP context. Infer the fictional date, location, and weather from the chat.
Decide if this memory should be secret (isSecret: true) if it contains very private thoughts.
...
async function manualGenerate() {
    ...
    $('#lumi-modal-body').html(`
        <div style="padding: 16px;">
            ...
            <div style="margin-bottom: 16px;">
                ...
                <div style="text-align: center; font-size: 11px; color: #ffb6c1;"><span id="range-value">${s.generation.messageRange}</span> ข้อความ</div>
            </div>
            <div style="display: flex; gap: 10px; margin-top: 20px;">
                <button id="gen-cancel" class="lumi-btn-sm" style="flex: 1;">ยกเลิก</button>
                <button id="gen-confirm" class="lumi-btn-gen" style="flex: 2;">✨ บันทึก</button>
            </div>
        </div>
    `);
    ...
    $('#gen-confirm').on('click', async () => {
        ...
        const res=await requestDiaryGen({trigger:'manual', linkedChars:wm==='rpg'?getChatNPCs(3):[], messageRange});
        
        if(res) {
            createMemoryEntry(res, 'manual', ctx, wm, '', ctx.chat?.length || 0);
            showToast('✅ บันทึกเสร็จแล้ว', 'success');
...
function createMemoryEntry(res, type, ctx, wm, refText, messageIndex) {
    const entry = {
        ...
        characterId: ctx.characterId, worldMode: wm,
        content: {
            rp_date: res.rp_date || "วันไม่ทราบแน่ชัด",
            rp_location: res.rp_location || "สถานที่",
    ...
    saveMemory(entry);
    showToast(`บันทึก: ${res.rp_date}`);
}

// ═══════════════════════════════════════════════
// STYLES (แก้ CSS ปุ่มลอยให้ลากได้)
// ═══════════════════════════════════════════════
function injectStyles() {
    ...
    s.innerHTML=`
        ...
        :root {
            --lumi-pink-soft: #FFF0F5; --lumi-pink-mid: #FFB6C1; --lumi-pink-bold: #FF69B4;
            --lumi-purple: #E6D5F0; --lumi-gold: #FFD700;
            --lumi-glass: rgba(255, 240, 245, 0.88); --lumi-shadow: 0 8px 32px rgba(255, 105, 180, 0.15);
        }
        ...
        #lumi-main-fab{
            ...
            background:var(--lumi-glass) url('${btnUrl}') no-repeat center center!important; backdrop-filter:blur(10px)!important;
            -webkit-backdrop-filter:blur(10px)!important; border:2px solid #FFB6C1!important; box-shadow:var(--lumi-shadow)!important;
            cursor:grab!important; 
            touch-action:none!important; -webkit-touch-callout:none!important;
        ...
        .lumi-menu-container{
            position:fixed; z-index:2147483646; display:none; background:rgba(255,255,255,0.96);
            backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); border-radius:24px; padding:20px;
            border:1.5px solid rgba(255,182,193,0.3); box-shadow:0 15px 40px rgba(255,182,193,0.2);
            font-family:'Mitr',sans-serif; font-weight:300; min-width:200px;
        }
        ...
        .lumi-memory-card.favorite{border-left:3px solid var(--lumi-gold)}
...
function openLumiModal(type) {
    ...
    else if(type==='phone') { $('#lumi-modal-title').text('Phone'); $('#lumi-modal-body').html('<div class="lumi-empty">Coming Soon</div>'); }
}
...
function renderDashboard() {
    ...
    const totalMem = mem.length, pinnedMem = mem.filter(m => m.meta.isPinned).length;
    const favoriteMem = mem.filter(m => m.meta.isFavorite).length, secretMem = mem.filter(m => m.meta.isSecret).length;
    ...
    $('#lumi-modal-body').html(`
        <div class="lumi-dashboard">
            <div class="lumi-stats-grid">
                ...
                <div class="lumi-stat-card"><div class="lumi-stat-value">${pinnedMem}</div><div class="lumi-stat-label">ปักหมุด</div></div>
                <div class="lumi-stat-card"><div class="lumi-stat-value">${favoriteMem}</div><div class="lumi-stat-label">ล้ำค่า ⭐</div></div>
                <div class="lumi-stat-card"><div class="lumi-stat-value">${secretMem}</div><div class="lumi-stat-label">ความลับ</div></div>
            </div>
            <div class="lumi-settings-card">
                <h4>ตัวละคร</h4>
                ${characters.map(char => `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;font-size:12px;color:#666;"><span>${escapeHtml(char)}</span><span style="color:#ff85a2;">${mem.filter(m => m.character === char).length}</span></div>`).join('')}
            </div>
        </div>
    `);
}
function renderSettingsModal() {
    ...
    const s = extension_settings[extensionName].diary, ag = s.autoGen;
    $('#lumi-modal-body').html(`
        <div style="padding: 16px;">
            <div class="lumi-settings-card">
                <h4>${svgSettings} ทั่วไป</h4>
                <div style="margin-bottom: 10px;"><label style="font-size: 11px; color: #666;">โหมดโลก</label><select id="modal-world-mode" class="lumi-input" style="margin-top: 4px;"><option value="auto" ${s.worldMode === 'auto' ? 'selected' : ''}>อัตโนมัติ</option><option value="solo" ${s.worldMode === 'solo' ? 'selected' : ''}>เดี่ยว</option><option value="rpg" ${s.worldMode === 'rpg' ? 'selected' : ''}>RPG</option></select></div>
                <div><label style="font-size: 11px; color: #666;">การแสดงผล</label><select id="modal-view-mode" class="lumi-input" style="margin-top: 4px;"><option value="timeline" ${s.display.viewMode === 'timeline' ? 'selected' : ''}>Timeline</option><option value="rpg" ${s.display.viewMode === 'rpg' ? 'selected' : ''}>RPG Columns</option></select></div>
            </div>
            <div class="lumi-settings-card">
                <h4>${svgRefresh} อัตโนมัติ</h4>
                <div class="lumi-toggle-row"><span style="font-size: 11px;">เปิดใช้งาน</span><div id="modal-autogen-toggle" class="lumi-toggle ${ag.enabled ? 'active' : ''}"></div></div>
                <div style="margin-top: 8px;"><label style="font-size: 11px; color: #666;">ทริกเกอร์</label><select id="modal-trigger-type" class="lumi-input" style="margin-top: 4px;"><option value="turn_count" ${ag.triggerType === 'turn_count' ? 'selected' : ''}>ทุก X ข้อความ</option><option value="emotion" ${ag.triggerType === 'emotion' ? 'selected' : ''}>คำอารมณ์</option><option value="random" ${ag.triggerType === 'random' ? 'selected' : ''}>สุ่ม</option></select></div>
                <div id="modal-turn-wrap" style="margin-top: 8px; ${ag.triggerType !== 'turn_count' ? 'display: none;' : ''}"><label style="font-size: 11px; color: #666;">เจนทุก</label><input type="number" id="modal-turn-interval" value="${ag.turnInterval}" min="5" max="100" class="lumi-input" style="width: 60px; margin-left: 6px;"> <span style="font-size: 10px; color: #888;">ข้อความ</span></div>
            </div>
            <div class="lumi-settings-card">
                <h4>${svgLock} ความลับ</h4>
                <label style="font-size: 11px; color: #666;">วิธีปลดล็อก</label><select id="modal-secret-mode" class="lumi-input" style="margin-top: 4px;"><option value="ai" ${s.display.secretUnlockMode === 'ai' ? 'selected' : ''}>AI ตัดสินใจ</option><option value="affection" ${s.display.secretUnlockMode === 'affection' ? 'selected' : ''}>≥ 80</option><option value="time" ${s.display.secretUnlockMode === 'time' ? 'selected' : ''}>3 วัน</option><option value="manual" ${s.display.secretUnlockMode === 'manual' ? 'selected' : ''}>แมนนวล</option></select>
            </div>
            <div style="margin-top: 16px; display: flex; gap: 8px;">
                <button id="modal-save" class="lumi-btn-gen" style="flex: 2;">💾 บันทึก</button>
                <button id="modal-reset-fab" class="lumi-btn-sm" style="flex: 1;">${svgReset} รีเซ็ต</button>
            </div>
        </div>
    `);
    ...
    $('#modal-save').on('click', () => {
        s.worldMode = $('#modal-world-mode').val(); s.display.viewMode = $('#modal-view-mode').val();
        s.autoGen.enabled = $('#modal-autogen-toggle').hasClass('active'); s.autoGen.triggerType = $('#modal-trigger-type').val();
        s.autoGen.turnInterval = parseInt($('#modal-turn-interval').val()) || 20; s.display.secretUnlockMode = $('#modal-secret-mode').val();
        SillyTavern.getContext().saveSettingsDebounced(); showToast('บันทึกแล้ว'); setTimeout(() => renderDiaryUI(), 500);
    });
    $('#modal-reset-fab').on('click', () => {
        ...
        if (fab.length) { fab.css(defaultPos).show(); saveFabPosition(defaultPos); showToast('รีเซ็ตแล้ว'); }
    });
}
...
function renderDiaryUI() {
    ...
    $('#lumi-modal-body').html(`
        ...
        <div id="lumi-diary-content">
            <div style="text-align:center;margin-bottom:12px"><button id="lumi-manual-gen" class="lumi-btn-gen">${svgPlus} บันทึก</button></div>
            <div class="lumi-filter-bar">
                <select id="lumi-f-char" class="lumi-filter-select"><option value="">ทุกตัวละคร</option>${chars.map(c=>`<option value="${escapeHtml(c.name)}" style="color: ${c.color}">${escapeHtml(c.name)}</option>`).join('')}</select>
...
function renderRPGView() {
    ...
    for(const[char, entries] of Object.entries(byChar)) {
        ...
        html += `<div class="lumi-character-column"><div class="lumi-character-header"><div class="lumi-character-avatar" style="background: ${charInfo.color}">${escapeHtml(char.charAt(0).toUpperCase())}</div><div><div class="lumi-character-name">${escapeHtml(char)}</div><div class="lumi-character-count">${entries.length} รายการ</div></div></div>${entries.map(m => renderMemoryCard(m, true)).join('')}</div>`;
    }
    html += '</div>';
    panel.html(html);
...
function renderMemoryCard(entry, compact=false) {
    ...
    const rp = `<span class="lumi-rp-info">${svgCalendar} ${escapeHtml(entry.content.rp_date)}</span><span class="lumi-rp-info">${svgMapPin} ${escapeHtml(entry.content.rp_location)}</span><span class="lumi-rp-info">${svgCloud} ${escapeHtml(entry.content.rp_weather)}</span>`;
    ...
    const acts = `<button class="lumi-btn-icon ${entry.meta.isPinned?'active':''}" data-act="pin" title="ปักหมุด">${svgPin}</button><button class="lumi-btn-icon ${entry.meta.isFavorite?'active':''}" data-act="fav" title="ล้ำค่า">${entry.meta.isFavorite ? svgStarFilled : svgStar}</button><button class="lumi-btn-icon" data-act="ref" title="บริบท">${svgRef}</button><button class="lumi-btn-icon danger" data-act="del" title="ลบ">${svgTrash}</button>`;
    if(isLocked) return `<div class="lumi-memory-card secret-locked" data-id="${entry.id}"><div class="lumi-locked-overlay">${svgLock}<div class="lumi-locked-text">ความทรงจำนี้ยังมองไม่เห็น</div><div class="lumi-locked-hint">ปลดล็อกเมื่อความสัมพันธ์ ≥ 80</div></div><div class="lumi-memory-header"><span class="lumi-memory-char"><span class="lumi-char-dot" style="background:${charInfo.color}"></span>${escapeHtml(entry.character)}</span></div><div class="lumi-memory-meta">${rp}</div></div>`;
    return `<div class="lumi-memory-card ${entry.meta.isPinned?'pinned':''} ${entry.meta.isFavorite?'favorite':''}" data-id="${entry.id}"><div class="lumi-memory-header"><span class="lumi-memory-char"><span class="lumi-char-dot" style="background:${charInfo.color}"></span>${escapeHtml(entry.character)}</span><div class="lumi-memory-meta">${compact ? '' : rp} ${refBadge}</div></div><div style="font-size:10px;color:#888;margin-bottom:6px">${entry.content.mood} · ❤️ ${entry.content.affection_score}</div><div class="lumi-memory-content">${escapeHtml(entry.content.diary)}</div><div class="lumi-memory-tags">${tags}</div>${compact ? '' : `<div class="lumi-memory-actions">${acts}</div>`}</div>`;
}

function bindMemoryCardEvents() {
    $('.lumi-btn-icon[data-act="pin"]').off('click').on('click',function(e){e.stopPropagation();const id=$(this).closest('.lumi-memory-card').data('id');togglePin(id);});
    $('.lumi-btn-icon[data-act="fav"]').off('click').on('click',function(e){e.stopPropagation();const id=$(this).closest('.lumi-memory-card').data('id');toggleFavorite(id);});
    $('.lumi-btn-icon[data-act="ref"]').off('click').on('click',function(e){e.stopPropagation();const id=$(this).closest('.lumi-memory-card').data('id');showRef(id);});
    $('.lumi-btn-icon[data-act="del"]').off('click').on('click',function(e){e.stopPropagation();const id=$(this).closest('.lumi-memory-card').data('id');delMem(id);});
    $('.lumi-ref-badge').off('click').on('click', function(e) { e.stopPropagation(); const msgIndex = $(this).data('msg-index'); if (msgIndex !== undefined) scrollToMessage(msgIndex); });
}
...
function toggleFavorite(id){const s=extension_settings[extensionName],m=s.memories.find(x=>x.id===id);if(m){m.meta.isFavorite=!m.meta.isFavorite;if(m.meta.isFavorite && !m.meta.tags.includes('#ล้ำค่า'))m.meta.tags.push('#ล้ำค่า');else m.meta.tags = m.meta.tags.filter(t => t !== '#ล้ำค่า');SillyTavern.getContext().saveSettingsDebounced();loadAndRenderTimeline();showToast(m.meta.isFavorite ? '⭐ เพิ่มเป็นล้ำค่า' : 'ลบออกแล้ว');}}
function showRef(id){const m=extension_settings[extensionName].memories.find(x=>x.id===id);if(m && m.meta.referencedMessageIndex !== undefined) scrollToMessage(m.meta.referencedMessageIndex); else alert('บริบท:\n'+(m?.meta.referenceText||'ไม่มีข้อมูล'));}
function delMem(id){if(confirm('ลบ?')){const s=extension_settings[extensionName];s.memories=s.memories.filter(x=>x.id!==id);SillyTavern.getContext().saveSettingsDebounced();loadAndRenderTimeline();}}
function exportMemories() {const data = JSON.stringify(extension_settings[extensionName].memories, null, 2);const blob = new Blob([data], {type: 'application/json'});const url = URL.createObjectURL(blob);const a = document.createElement('a');a.href = url;a.download = `lumipulse-${new Date().toISOString().split('T')[0]}.json`;a.click();URL.revokeObjectURL(url);showToast('Export สำเร็จ');}
...
function createSettingsUI() {
    ...
    $('#extensions_settings').append(`
        <div id="lumi-settings-drawer" class="inline-drawer"><div class="inline-drawer-toggle inline-drawer-header"><b style="color:#ff85a2; font-family:'Mitr'; font-weight:300;">LumiPulse</b><div class="inline-drawer-icon fa-solid fa-circle-chevron-down down"></div></div>
        <div class="inline-drawer-content" style="display: none;"><div style="margin-bottom: 12px;"><label class="checkbox_label"><input id="lumi_enable_toggle" type="checkbox" ${extension_settings[extensionName].isEnabled ? 'checked' : ''} /><span>เปิดใช้งาน</span></label></div>
        <div style="font-size: 11px; color: #ffb6c1; margin-bottom: 10px; border-bottom: 1px solid #333; padding-bottom: 5px;">ทั่วไป</div>
        <div style="margin-bottom: 8px;"><label style="font-size: 12px; color: #aaa;">โหมดโลก</label><select id="lumi_world_mode" class="text_pole"><option value="auto" ${s.worldMode === 'auto' ? 'selected' : ''}>อัตโนมัติ</option><option value="solo" ${s.worldMode === 'solo' ? 'selected' : ''}>เดี่ยว</option><option value="rpg" ${s.worldMode === 'rpg' ? 'selected' : ''}>RPG</option></select></div>
        <div style="margin-bottom: 8px;"><label style="font-size: 12px; color: #aaa;">การแสดงผล</label><select id="lumi_view_mode" class="text_pole"><option value="timeline" ${s.display.viewMode === 'timeline' ? 'selected' : ''}>Timeline</option><option value="rpg" ${s.display.viewMode === 'rpg' ? 'selected' : ''}>RPG Columns</option></select></div>
        <div style="margin-bottom: 8px;"><label class="checkbox_label" style="font-size: 12px; color: #aaa;"><input id="lumi_autogen_toggle" type="checkbox" ${ag.enabled ? 'checked' : ''} /><span>เจนอัตโนมัติ</span></label></div>
        <div id="lumi_autogen_options" style="display: ${ag.enabled ? 'block' : 'none'}; margin-left: 15px; margin-bottom: 10px;"><label style="font-size: 11px; color: #888;">ทริกเกอร์:</label><select id="lumi_trigger_type" class="text_pole" style="width: 100%; margin: 4px 0;"><option value="turn_count" ${ag.triggerType === 'turn_count' ? 'selected' : ''}>ทุก X ข้อความ</option><option value="emotion" ${ag.triggerType === 'emotion' ? 'selected' : ''}>คำอารมณ์</option><option value="random" ${ag.triggerType === 'random' ? 'selected' : ''}>สุ่ม</option></select><div id="lumi_turn_interval_wrap" style="display: ${ag.triggerType === 'turn_count' ? 'block' : 'none'}; margin-top: 4px;"><input id="lumi_turn_interval" type="number" value="${ag.turnInterval}" min="5" max="100" class="text_pole" style="width: 60px;"> <span style="font-size: 11px; color: #888;">ข้อความ</span></div></div>
        <div style="font-size: 11px; color: #ffb6c1; margin: 10px 0; border-bottom: 1px solid #333; padding-bottom: 5px;">การเจน</div><div style="margin-bottom: 8px;"><label style="font-size: 12px; color: #aaa;">อ่านกี่ข้อความ</label><input id="lumi_gen_range" type="number" value="${s.generation.messageRange}" min="5" max="100" class="text_pole" style="width: 60px; margin-top: 4px;"></div>
        <div style="margin-top: 15px; border-top: 1px solid #333; padding-top: 10px;"><button id="lumi_reset_fab" class="menu_button" style="margin-bottom: 5px;">${svgReset} รีเซ็ตปุ่มลอย</button><button id="lumi_export" class="menu_button" style="margin-bottom: 5px;">${svgDownload} Export JSON</button><button id="lumi_clear_memories" class="menu_button danger">${svgTrash} ล้างทั้งหมด</button></div></div></div>
    `);
    $('#lumi_enable_toggle').on('change', function() { extension_settings[extensionName].isEnabled = $(this).prop('checked'); SillyTavern.getContext().saveSettingsDebounced(); if(extension_settings[extensionName].isEnabled) { setTimeout(() => { spawnLumiButton(); createContentModal(); setupAutoTriggerListener(); }, 500); } else { $('#lumi-main-fab, .lumi-menu-container, #lumi-modal-overlay').remove(); $(document).off('messageReceived', onNewChat); } });
    ...
    $('#lumi_reset_fab').on('click', function() { const fab = $('#lumi-main-fab'); const defaultPos = { top: '50%', right: '0px', left: 'auto', bottom: 'auto', transform: 'translateY(-50%)' }; if (fab.length) { fab.css(defaultPos).show(); saveFabPosition(defaultPos); showToast('รีเซ็ตแล้ว'); } else { spawnLumiButton(); showToast('สร้างใหม่แล้ว'); } });
    $('#lumi_export').on('click', () => exportMemories());
    $('#lumi_clear_memories').on('click', function() { if(confirm('ล้าง?')) { extension_settings[extensionName].memories = []; SillyTavern.getContext().saveSettingsDebounced(); showToast('ล้างแล้ว'); } });
}

// ═══════════════════════════════════════════════
// FAB BUTTON (แก้ให้ลากได้จริง)
// ═══════════════════════════════════════════════
function spawnLumiButton() {
    ...
    if (pos && pos.left && pos.top && !pos.transform?.includes('translateY')) {
        fab.style.left = pos.left; fab.style.top = pos.top; fab.style.right = 'auto'; fab.style.bottom = 'auto'; fab.style.transform = 'none';
    } else {
        fab.style.top = '50%'; fab.style.right = '0px'; fab.style.left = 'auto'; fab.style.bottom = 'auto'; fab.style.transform = 'translateY(-50%)';
    }
    ...
    menu.innerHTML = `
        <div class="lumi-menu-header"><span class="lumi-menu-title">LumiPulse</span><div class="lumi-menu-close" id="lumi-menu-close">${svgClose}</div></div>
    ...
    function updateMenuPos() {
        ...
        if (l < 10) l = 10; if (l + m.outerWidth() > window.innerWidth - 10) l = window.innerWidth - m.outerWidth() - 10;
    ...
    const TH = 8;
    let tapTimer = null, hasMoved = false, fabRect = null;
    ...
    fab.addEventListener('mousedown', e => {
        ...
        offset.x = e.clientX - (fabRect.left + fabRect.width / 2);
        offset.y = e.clientY - (fabRect.top + fabRect.height / 2);
        dragStart.x = e.clientX; dragStart.y = e.clientY;
        currentPos.x = fabRect.left; currentPos.y = fabRect.top;
        ...
        const onMove = (ev) => {
            ...
            if (dist > TH && !hasMoved) { hasMoved = true; isDragging = true; $(menu).fadeOut(50); }
            if (!isDragging) return;
            currentPos.x = Math.max(0, Math.min(ev.clientX - offset.x - (fabRect.width/2), window.innerWidth - fabRect.width - 5));
            currentPos.y = Math.max(0, Math.min(ev.clientY - offset.y - (fabRect.height/2), window.innerHeight - fabRect.height - 5));
        ...
        const onUp = (ev) => {
            ...
            if (!isDragging && !hasMoved) {
                clearTimeout(tapTimer); tapTimer = setTimeout(() => { updateMenuPos(); $(menu).fadeToggle(150); spawnHeartEffect({ ...ev, _fromDrag: false }); }, 50);
            } else if (isDragging) {
                saveFabPosition({ top: currentPos.y + 'px', right: 'auto', left: currentPos.x + 'px', bottom: 'auto', transform: 'none' });
            }
            isDragging = false; hasMoved = false;
        };
    ...
    fab.addEventListener('touchstart', e => {
        e.preventDefault(); e.stopPropagation();
        isTouchDrag = false; hasMoved = false;
        fab.classList.add('dragging');
        const touch = e.touches[0]; fabRect = fab.getBoundingClientRect();
        offset.x = touch.clientX - (fabRect.left + fabRect.width / 2);
        offset.y = touch.clientY - (fabRect.top + fabRect.height / 2);
    ...
    fab.addEventListener('touchmove', e => {
        ...
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist > TH && !hasMoved) { hasMoved = true; isTouchDrag = true; $(menu).fadeOut(50); }
        if (!isTouchDrag) return;
        currentPos.x = Math.max(0, Math.min(touch.clientX - offset.x - (fabRect.width/2), window.innerWidth - fabRect.width - 5));
        currentPos.y = Math.max(0, Math.min(touch.clientY - offset.y - (fabRect.height/2), window.innerHeight - fabRect.height - 5));
        fab.style.left = currentPos.x + 'px'; fab.style.top = currentPos.y + 'px';
    }, { passive: false });
    ...
    fab.addEventListener('touchend', e => {
        ...
        if (!isTouchDrag && !hasMoved) {
            clearTimeout(tapTimer); tapTimer = setTimeout(() => { updateMenuPos(); $(menu).fadeToggle(150); const touch = e.changedTouches?.[0]; if (touch) spawnHeartEffect({ _fromDrag: false, clientX: touch.clientX, clientY: touch.clientY }); }, 50);
        } else if (isTouchDrag) {
            saveFabPosition({ top: currentPos.y + 'px', right: 'auto', left: currentPos.x + 'px', bottom: 'auto', transform: 'none' });
        }
        isTouchDrag = false; hasMoved = false;
