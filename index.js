...
const defaultSettings = {
    ...
    memories: [],
    _internal: { messageCounter: 0, firstChatDate: null, fabReset: false },
    diary: {
        worldMode: 'auto',
...
function initLumiPulse() {
    ...
    if (s.isEnabled) {
        // หน่วงนิดหน่อยให้ UI ของ ST โหลดโครงสร้างเสร็จก่อน
        setTimeout(() => {
            spawnLumiButton();
            createContentModal();
...
async function onNewChat() {    const s=extension_settings[extensionName], cfg=s.diary.autoGen;
...
// ═══════════════════════════════════════════════
// STYLES (บังคับแสดงผลปุ่มลอย 100%)
// ═══════════════════════════════════════════════
function injectStyles() {
    ...
    s.innerHTML=`
        ...
        #lumi-main-fab{
            ...
            background:rgba(255,240,245,0.92) url('${btnUrl}') no-repeat center center!important; backdrop-filter:blur(10px)!important;
            border:2px solid #FFB6C1!important; box-shadow:0 6px 20px rgba(255,182,193,0.45)!important;
        ...
        .lumi-menu-container{position:fixed;z-index:2147483646;display:none;background:rgba(255,255,255,0.95);backdrop-filter:blur(20px);border-radius:28px;padding:22px;border:1.5px solid rgba(255,182,193,0.3);box-shadow:0 15px 40px rgba(255,182,193,0.2);font-family:'Mitr',sans-serif;font-weight:300}
        ...
        .lumi-modal-overlay{position:fixed;top:0;left:0;width:100vw;height:100dvh;background:rgba(0,0,0,0.25);backdrop-filter:blur(12px);z-index:2147483648;display:none;align-items:center;justify-content:center}
        ...
        .lumi-memory-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;flex-wrap:wrap;gap:5px}        .lumi-memory-char{font-weight:400;color:#444;font-size:14px}
...
function createContentModal() {
    ...
    $('body').append(`<div id="lumi-modal-overlay" class="lumi-modal-overlay"><div class="lumi-modal-box"><div class="lumi-modal-header"><span id="lumi-modal-title"></span><div class="lumi-modal-close">×</div></div><div id="lumi-modal-body" class="lumi-modal-body"></div></div></div>`);
...
function renderDiaryUI() {
    ...
    $('#lumi-modal-body').html(`
        ...
        <div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap">
            ...
            <input id="lumi-f-search" class="text_pole" style="flex:2" placeholder="🔍 ค้นหาไดอารี่...">        </div>
...
function createSettingsUI() {
    if ($('#lumi-settings-drawer').length) return;    const s = extension_settings[extensionName].diary;
    const ag = s.autoGen;
    ...
    $('#lumi_reset_fab').on('click', function() {
        ...
        if (fab.length) {
            fab.css({ top: '50%', right: '0px', left: 'auto', bottom: 'auto', transform: 'translateY(-50%)' }).show();
...
// ═══════════════════════════════════════════════
// 🛡️ FAB BUTTON (รับประกันการแสดงผล)
// ═══════════════════════════════════════════════
function spawnLumiButton() {
    $('#lumi-main-fab, .lumi-menu-container').remove();
    if (!document.body) { console.warn("[Lumi] body not ready, retrying..."); setTimeout(spawnLumiButton, 800); return; }
    
    console.log("[Lumi] 🔄 Creating FAB...");
    const fab = document.createElement('div');
    fab.id = 'lumi-main-fab';
    // ตำแหน่งเริ่มต้น: กึ่งกลางขอบขวา
    fab.style.top = '50%';
    fab.style.right = '0px';
    ...
    setTimeout(() => { 
        ...
        fab.style.opacity = '1'; 
        console.log("[Lumi] ✅ FAB Appended & Visible");
    }, 50);
    ...
    fab.addEventListener('mousedown', e => {
        ...
        const onMove = ev => {
            ...
            if (Math.sqrt(dx * dx + dy * dy) > TH) {
                isDragging = true;
                $(menu).fadeOut(100);
                // ลบ transform ออกครั้งแรกที่ลาก เพื่อให้ left/top ทำงานได้ถูกต้อง
                fab.style.transform = 'none';                fab.style.right = 'auto';
                fab.style.top = 'auto';
            }
    ...
    fab.addEventListener('touchmove', e => {
        ...
        let x = Math.max(0, Math.min(t.clientX - offset.x, window.innerWidth - 48));        let y = Math.max(0, Math.min(t.clientY - offset.y, window.innerHeight - 48));
        fab.style.left = x + 'px';
        fab.style.top = y + 'px';
    }, { passive: false });
