...
const defaultSettings = { isEnabled: false, forumTopic: "", isForumInitialized: false, includeRandomNPCs: true };
...
function getChatNPCs() {
    ...
    const names = new Set();
    chat.slice(-30).forEach(m => { if (m.name && !m.is_user && !m.is_system) names.add(m.name); });
...
function injectStyles() {
    ...
    style.innerHTML = `
        ...
        .lumi-menu-item { display: flex; flex-direction: column; align-items: center; gap: 10px; cursor: pointer; transition: all 0.3s ease; }
        ...
        .lumi-modal-overlay { position: fixed; top: 0; left: 0; width: 100vw; height: 100dvh; background: rgba(0,0,0,0.15); backdrop-filter: blur(15px); z-index: 2147483648; display: none; align-items: center; justify-content: center; }
        .lumi-modal-box { width: 94%; max-width: 460px; height: 82vh; background: #FFFFFF; border-radius: 45px; border: 2px solid #FFD1DC; box-shadow: 0 30px 70px rgba(255,182,193,0.2); display: flex; flex-direction: column; overflow: hidden; font-family: 'Mitr', sans-serif; font-weight: 300; animation: lumiPop 0.4s forwards; }
        ...
        .lumi-input { width: 100%; background: #FFF9FA; border: 1.5px solid #FFD1DC; border-radius: 20px; padding: 15px; color: #ff85a2; font-family: 'Mitr'; text-align: center; outline: none; }
        .lumi-btn-gen { background: linear-gradient(135deg, #FFB6C1, #FF85A2); color: white; border: none; padding: 12px 45px; border-radius: 25px; font-family: 'Mitr'; cursor: pointer; transition: 0.3s; }
        ...
        .post-avatar { width: 45px; height: 45px; border-radius: 50%; border: 2px solid #FFD1DC; background: #FFF0F3; display: flex; align-items: center; justify-content: center; color: #ff85a2; font-weight: 400; overflow: hidden; }
        .post-author { font-weight: 400; color: #444; font-size: 15px; }
        .post-content { font-size: 14px; color: #666; line-height: 1.5; }
...
async function requestAIGeneration(topic, npcs, includeRandom) {
    ...
    const prompt = `[Task: Social Media Simulation]
Topic: "${topic}"
Current NPCs in chat: [${npcList}]
Instruction: Generate 3-4 forum/social media posts based on the topic and the current story context. 
- Use the names of existing NPCs.
${includeRandom ? "- You may invent new background characters if needed." : "- Use ONLY the NPCs provided."}
- Format: JSON Array ONLY. 
- Each object: {"author": "Name", "content": "Post text", "likes": number, "time": "e.g. 5m ago"}
Example: [{"author": "John", "content": "I saw something!", "likes": 10, "time": "2m ago"}]`;

    try {
        const response = await fetch('/api/generate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': window.SillyTavern.getContext().csrfToken },
            body: JSON.stringify({ prompt, model_role: "assistant", max_context_length: 2000, max_gen_length: 500, stop: ["\n\n"] })
        });
...
function openLumiModal(type) {
    ...
    if (type === 'forum') {
        ...
        $('#lumi-modal-title').text('Coming Soon');
        $('#lumi-modal-body').html('<div style="text-align:center; padding-top:100px; color:#ffb6c1;">Features currently in development. 🌸</div>');
    }
}
    ...
    if (!s.isForumInitialized) {
        body.html(`
            <div class="lumi-setup">
                <img src="${iconForum}" style="width:70px; margin-bottom:10px;">
                <div style="color:#ff85a2; font-size:16px;">ระบุหัวข้อฟอรั่มที่คุณต้องการสแกน</div>
                <input id="topic-input" class="lumi-input" placeholder="เช่น เรื่องวุ่นๆ ในคณะ, ข่าวลือผีในมหาลัย" />
                <label class="checkbox_label" style="font-size:13px; color:#ffb6c1;">
                    <input id="npc-opt" type="checkbox" ${s.includeRandomNPCs ? 'checked' : ''} />
                    <span>อนุญาตให้ AI สร้าง NPC เสริมอื่นๆ</span>
                </label>
                <button id="btn-gen" class="lumi-btn-gen">เริ่มการประมวลผล</button>
            </div>
        `);
        $('#btn-gen').on('click', async () => {
        ...
        body.html('<div class="lumi-loader-wrap"><div class="lumi-loader"></div><div style="color:#ff85a2;">AI กำลังอ่านแชทและสร้างโพสต์...</div></div>');
        const npcs = getChatNPCs();
        requestAIGeneration(s.forumTopic, npcs, s.includeRandomNPCs).then(posts => {
            body.empty();
            body.append(`<div style="font-size:11px; color:#ffb6c1; text-align:center; margin-bottom:15px;">Browsing: ${s.forumTopic}</div>`);
            if (posts.length === 0) body.append('<div style="text-align:center; color:#ffb6c1;">AI ไม่สามารถประมวลผลได้ในขณะนี้</div>');
            posts.forEach(p => {
                body.append(`
                    <div class="forum-post">
                        <div class="post-header">
                            ...
                            <div style="display:flex; flex-direction:column;">
                                ...
                                <span style="font-size:10px; color:#CCC;">${p.time}</span>
                            </div>
                        </div>
                        <div class="post-content">${p.content}</div>
                        <div class="post-footer">❤️ ${p.likes} คนถูกใจสิ่งนี้</div>
                    </div>
                `);
            });
        });
...
function createSettingsUI() {
    ...
    $(document).on('click', '#lumi-reset', () => { extension_settings[extensionName].isForumInitialized = false; SillyTavern.getContext().saveSettingsDebounced(); toastr.success("รีเซ็ตหัวข้อฟอรั่มแล้ว"); });
    }
